...
async function tryGenerateResumeObject(modelName: string, parsedText: string) {
  try {
    ...
    console.log(`Successfully generated structured resume data with model: ${modelName}`);
    return object;
  } catch (error: any) {
  ...
  try {
    ...
    try {
      ...
      if (primaryError.response?.status === 429) {
        ...
        try {
          // 🤖 Step 5: Use AI to parse the extracted text into a structured JSON format
          const object = await tryGenerateResumeObject(models.fallback, parsed.text);
          return NextResponse.json(object);
        } catch (fallbackError: any) {
          console.error("Fallback model failed:", fallbackError);
          return NextResponse.json({ error: "Fallback AI model failed to parse resume." }, { status: 500 });
        }
      }
      console.error("Primary model failed:", primaryError);
      return NextResponse.json({ error: "Primary AI model failed to parse resume." }, { status: 500 });
    }
  } catch (pdfErr) {
